...
import (
	...
	"github.com/xiaoenai/ants/cmd/ant/run/fsnotify"
)
...
func RunProject(newWatchExts []string, newNotWatch []string) {
	...
	tp.Printf("[ant] Initializing watcher...")
...
func getFileModTime(path string) int64 {
	...
	f, err := os.Open(path)
	if err != nil {
		tp.Errorf("[ant] Fail to open file[ %s ]", err)
	...
	if err != nil {
		tp.Errorf("[ant] Fail to get file information[ %s ]", err)
		return time.Now().Unix()
	}
...
func rewatch() {
	...
	if err != nil {
		tp.Errorf("[ant] Fail to create new watcher[ %v ]", err)
		os.Exit(2)
	}
	for _, path := range readAppDirectories("./") {
		tp.Printf("[ant] Watching directory[ %s ]", path)
		err = watcher.Watch(path)
		if err != nil {
			tp.Errorf("[ant] Fail to watch curpathectory[ %s ]", err)
			os.Exit(2)
		}
	}
	...
	for {
		...
		case err := <-watcher.Error:
			tp.Warnf("[ant] %s", err.Error()) // No need to exit here
		}
	}
}
...
func rerun() {
	...
	err := buildCom.Run()
	if err != nil {
		tp.Errorf("[ant] ============== Build failed ===================")
		return
	}
	tp.Printf("[ant] Build was successful")

	var start string
	if isFirstStart {
		isFirstStart = false
		tp.Printf("[ant] Starting app: %s", info.ProjName())
		start = "Start"
	} else {
		tp.Printf("[ant] Restarting app: %s", info.ProjName())
		defer func() {
			if e := recover(); e != nil {
				tp.Printf("[ant] Kill.recover -> %v", e)
			}
		}()
	...
	go func() {
		...
		if err := cmd.Start(); err != nil {
			tp.Errorf("[ant] Fail to start app[ %s ]", err)
			return
		}
		tp.Printf("[ant] %s was successful", start)
		cmd.Wait()
		tp.Printf("[ant] Old process was stopped")
	}()
}
...
func readAppDirectories(dir string) (paths []string) {
	...
	if err != nil {
		tp.Fatalf("[ant] read project directorys failed: %v", err)
		return
	}
	useDirectory := false
