		...
		for _, spec := range genDecl.Specs {
			...
			case *ast.StructType, *ast.Ident:
				...
				if name == MODEL {
					...
					if !ok {
						tp.Fatalf("[micro] the type of %s must be struct", MODEL)
					}
					p.parseToModels(mod)
					continue
				}
				...
				if !goutil.IsExportedName(name) {
					tp.Fatalf("[micro] Unexported types: %s", name)
				}
				...
				for _, method := range t.Methods.List {
					...
					case *ast.FuncType:
						...
						if tt.Params.NumFields() != 1 {
							tp.Fatalf("[micro] Invalid method: %s", subName)
							continue
						}
						dst := bytes.NewBuffer(nil)
...
func (p *Project) fillFile(k string) {
	...
	case "logic/tmp_code.gen.go":
		codeFiles[k] = "// Code generated by 'micro gen' command.\n" +
...
func (p *Project) parseToModels(mod *ast.StructType) {
	for _, field := range mod.Fields.List {
		...
		if !ok {
			...
			tp.Fatalf("[micro] the field %s of %s must be struct", t.Name, MODEL)
		}
		p.Models[t.Name] = newModel(t.Name)
	}
...
func (p *Project) Generator() {
	...
	mustMkdirAll("sdk")
	// write files
	notFirst := goutil.FileExists(defMicroGenLock)
	if !notFirst {
		tpl.RestoreAsset("./", defMicroGenLock)
	}
