...
func (p *Project) fillFile(k string) {
	...
	switch k {
	case "main.go", "config.go", "go.mod", "logic/model/init.go", "logic/tools/base_config.go":
		p.codeFiles[k] = v
...
func hasGenSuffix(name string) bool {
	switch name {
	case "README.md", ".gitignore", "main.go", "config.go", "go.mod", "args/const.go",
...
func (p *Project) Generator(force, newdoc bool) {
	...
	for k, v := range p.codeFiles {
		...
		if k != "go.mod" {
			...
			f.Write(goutil.StringToBytes(v))
		}
		f.Close()
...
func (p *Project) genMainFile() {
	...
	p.replace("go.mod", "${service_api_prefix}", goutil.SnakeString(p.Name))
}
