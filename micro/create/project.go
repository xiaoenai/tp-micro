...
func (p *Project) genTypesFile() {
	replaceWithLine("args/type.gen.go", "${import_list}", p.tplInfo.TypeImportString())
	replaceWithLine("args/type.gen.go", "${type_define_list}", p.tplInfo.TypesString())
}
...
func (p *Project) genHandlerFile() {
	...
	replaceWithLine("api/push_handler.gen.go", "${handler_api_define}", s)
	...
	replaceWithLine("api/pull_handler.gen.go", "${handler_api_define}", s)
}
...
func (p *Project) genLogicFile() {
	...
	replaceWithLine("logic/tmp_code.gen.go", "${logic_api_define}", s)
}
...
func (p *Project) genSdkFile() {
	...
	replaceWithLine("sdk/rpc.gen.go", "${rpc_call_define}", s1)
	replaceWithLine("sdk/rpc.gen_test.go", "${rpc_call_test_define}", s2)
}
...
func (p *Project) genModelFile() {
	for _, m := range p.tplInfo.models.mysql {
		...
		codeFiles[fileName] = newModelString(m)
		p.fillFile(fileName)
	}
