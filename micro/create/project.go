...
func (p *Project) fillFile(k string) {
	...
	if strings.Contains(k, ".gen.") || strings.Contains(k, ".gen_test.") {
		p.codeFiles[k] = "// Code generated by 'micro gen' command.\n// DO NOT EDIT!\n\n" + v
	} else if strings.Contains(k, ".tmp.go") {
		p.codeFiles[k] = "// Temporary file used to ensure successful compilation!\n// It will be overwritten when `micro gen` is run!\n// When the project is completed, it should be removed!\n\n" + v
	} else {
		p.codeFiles[k] = v
	}
}
...
func (p *Project) Generator(force, newdoc bool) {
	...
	for k, v := range p.codeFiles {
		if !force &&
			!strings.Contains(k, ".gen.") &&
			!strings.Contains(k, ".gen_test.") &&
			!strings.Contains(k, ".tmp.go") {
			continue
		}
